...
To extract latent eye features from an image using EllSeg, use the new `export_libtorch.py` helper.

Example usages:

```bash
python export_libtorch.py --command export --weights ./weights/all.git_ok --output ./weights/ellseg_ritnet_v3.pt
python export_libtorch.py --command extract --weights ./weights/all.git_ok --image ./data/eye.png --output ./data/eye_features.npy
```

This script can:

* export an EllSeg model to a LibTorch-compatible TorchScript module
* load a single eye image, preprocess it to 240x320, and extract
  segmentation output + latent eye features + ellipse regression output
