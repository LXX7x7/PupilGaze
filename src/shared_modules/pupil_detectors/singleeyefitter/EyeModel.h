    ...
    */
    ...
    public:
            ...
            mObservation2D(observation)
        {
				...
				//test --- 已验证，共线，关键技术里面有证明
				/*{
					const auto& c2 = mUnprojectedCirclePair.second.center;
					const auto& v2 = mUnprojectedCirclePair.second.normal;
					Vector2 cProj2 = project(c2, focalLength);
					Vector2 vProj2 = project(v2 + c2, focalLength) - cProj2;
					vProj2.normalize();
					mProjectedCircleGaze = Line(cProj2, vProj2);

					Vector2 temp = cProj2 - cProj;
					temp.normalize();
					std::cout << "C 1:" << mUnprojectedCirclePair.first << "  C 2:" << mUnprojectedCirclePair.second << std::endl;
					std::cout << "P to P :" << temp << "  normal 1:" << vProj  << "  normal 2:" << vProj2 << std::endl;
					}
				*/
...
class EyeModel {
    ...
    public:
		...
		//*******************************************************
		//功    能： 【根据瞳孔的观测信息2D以及平均帧率，计算瞳孔3D信息，以及检测的置信度以及置信度的权重】
		//            
		//输入参数： 
		//     [in]          observation                               2D 瞳孔的观测信息
		//     [in]          averageFramerate                          当前取像的平均帧率
		//返 回 值：
		//     计算瞳孔3D信息，以及检测的置信度以及置信度的权重
		//修改日志：
		//*******************************************************
        std::pair<Circle,ConfidenceValue> presentObservation(const ObservationPtr observation, double averageFramerate  );

		//*******************************************************
		//功    能： 【获得当前用的球，即经过refineWithEdges函数进行优化的眼球】
		//            备注：优化方式是让瞳孔边界点离3D瞳孔投影2D图像上的边界最小；
		//输入参数： 
		//
		//返 回 值：
		//     返回优化后，即当前用的球
		...
		//*******************************************************
		//功    能： 【获得粗略计算的眼球，利用瞳孔法向量交出眼球中心，半径是瞳孔中心到球心的距离】
		//            
		//输入参数： 
		//
		//返 回 值：
		...
		//*******************************************************
		//功    能： 【获得瞳孔位置在拟合眼球中的均匀分布程度，返回值越大，说明采集的样本越充足】
		//            
		//输入参数： 
		//   
		//返 回 值：
    ...
    private:
		...
		//*******************************************************
		//功    能： 【内，被initialiseModel调用】【计算眼球中心，Z轴=57和半径=1内部固定】
		//            主要使用mSupportingPupils成员变量中的信息，并且此处确定2D瞳孔反向投影的唯一圆；
		//输入参数： 
		//    [in]    use_ransac      是否使用ransac算法拟合球心在二维的投影；
		//返 回 值：
		//     Sphere 返回眼球，z=57，r=1；
		//修改日志：
		//*******************************************************
        Sphere findSphereCenter( bool use_ransac = true);

		//*******************************************************
		//功    能： 【计算眼球中心，Z轴根据球的半径确定和半径内部固定=12】
		//            主要使用mSupportingPupils成员变量中的信息，此处获得initialiseModel[i].mparams的表示；
		//输入参数： 
		//    
		//返 回 值：
		//     Sphere 返回眼球半径为12mm，其他根据激活获得；
		//修改日志：
		//*******************************************************
        Sphere initialiseModel();

		//*******************************************************
		//功    能： 【1、利用图像瞳孔边缘点到3D圆重投影边界的距离最小，优化球心+3*3D圆参数，2、计算优化前后视线向量的的一致性，并返回】
		//            此处使用了ceres
		//输入参数： 
		//    [in/out]    sphere       眼球,其实内部有用mSupportingPupils
		//
		//返 回 值：
		//     计算优化前后视线向量的的一致性
		//修改日志：
		...
		//*******************************************************
		//功    能： 【瞳孔所有的信息包括2D和3D，导入到mSupportingPupils中】
		//            mSupportingPupilsToAdd --> mSupportingPupils
		//            此处需要获得线程锁，因为mSupportingPupils不支持多线程同时访问；
		//            mSupportingPupils会在用重建眼球模型中
		//输入参数： 
		//    
		//
		//返 回 值：
		...
		//*******************************************************
		//功    能： 【给检测的瞳孔3D位置打一个分数；1、分数值（方向投影和眼球交线处在的两个3D圆的normal应该对齐是最好的）；
		//                                           2、权重（相机和眼球中心的连线与initialisedCircle法向量之间的角度，角度越大越好，（因为如果角度为零，说明我们人眼此时正在看相机））】
		//            
		//输入参数： 
		//    [in]     unprojectedCircle                            2d椭圆反向投影后确定的3D圆（备注：其中一个指向球心的被干掉了）
		//    [in]     initialisedCircle                            相机中心和瞳孔中心的连线与眼球的交点，确定的3D圆；
		//    [in]     confidence                                   参与权重的计算；
		//返 回 值：
		//     返回3D瞳孔检测得分以及相应得分的权重
		//修改日志：
		//*******************************************************
		...
		//*******************************************************
		//功    能： 【根据图像采集的帧率，更新mPerformance窗口的大小，计算出mPerformanceGradient值，即检测性能变换的梯度】
		//            
		//输入参数： 
		//    [in]     observation_fit                            一个瞳孔3D位置检测的分数和权值
		//    [in]     averageFramerate                           平均帧率
		//返 回 值：
		//     返回3D瞳孔检测得分以及相应得分的权重
		//修改日志：
		//*******************************************************
		...
		//*******************************************************
		//功    能： 【检测瞳孔在3D空间中的相关性，如果此瞳孔位置以前存在，则返回false，不存在返回true】
		//            
		//输入参数： 
		//    [in]     circle                             反向投影的3D圆
		//返 回 值：
		...
		//*******************************************************
		//功    能： 【确定反向投影圆中的一个】
		//            球心和瞳孔方向在图像平面的投影的乘积确定；
		//输入参数： 
		//    [in]     sphere                             眼球
		//    [in]     circles                            瞳孔反向投影的圆（2个）；    
		//返 回 值：
		//     返回OK的圆
		//修改日志：
		//*******************************************************
		...
		//*******************************************************
		//功    能： 【根据初始化好的眼球（中心和半径已知）和瞳孔中心与相机中心连线的交点，重新优化瞳孔信息（3D圆，瞳孔参数）】
		//            
		//输入参数： 
		//    [in]     sphere                             眼球
		//    [in/out]     pupil                          瞳孔信息；    
		...
		//*******************************************************
		//功    能： 【根据瞳孔和相机中心直线，和眼球的交点（选择其中一个）作为3D瞳孔位置】
		//            3D瞳孔位置可以通过直线和球的交点获得；
		//输入参数： 
		//    [in]     sphere                             眼球
		//    [in]     circles                            瞳孔反向投影的圆（1个）；    
		//返 回 值：
		//     返回OK的圆
		//修改日志：
		//*******************************************************
        ...
        std::unordered_map<Vector2, bool, math::matrix_hash<Vector2>> mSpatialBins;           // 用来存储3D瞳孔法向量，用来检测新来的3D瞳孔是否在原来的集合中存在
        ...
        double mPerformanceGradient;   // Performance表现的后向差分，分母为当前时间-mLastPerformanceCalculationTime
		...
		const int mInitialUncheckedPupils;            // 起到保护作用，防止瞳孔拟合眼球是的数据过少，此值默认设置为3
		const double mBinResolution;                  // 检测瞳孔法向量的表示粒度（默认设置为0.05）
		const int mTotalBins;                         // 即瞳孔不同位置的个数；
		const int mModelID;                           // 用来标记眼球模型
		double mBirthTimestamp;                       // 此模型声明的时间
        ...
        std::vector<Pupil> mSupportingPupilsToAdd;    // 添加瞳孔信息；
};
