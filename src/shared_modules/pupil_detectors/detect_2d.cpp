...
int  Detector2D::coarse_center1(cv::Mat m, int min_, int max_, cv::Rect& roi)
{
	std::vector<HaarResponse> haarResult;
	double best_response = 10000;
	cv::Mat_<uchar> mEye;
	m.copyTo(mEye);                 // ת���ɻҶ�ͼ�񣬴˴��ⲿ�Ѿ�ת��OK
	//cv::imshow("ԭʼͼ", mEye);
	...
	for (int r = min_; r < max_; r += rstep)
	{
		...
		for (int y = r; y < mEye.rows - r - r - 1; y += ystep)
		{
			//            ?        ?                    
			...
			for (int x = r; x < mEye.cols - r; x += xstep)
			{
				...
				if (response < best_response)
				{
					...
					hr.minValueIn.y = y;
					hr.raduis = r;
					haarResult.push_back(hr);
					if (haarResult.size() > 15)
					{
						std::vector<HaarResponse>::iterator it = haarResult.begin();
						haarResult.erase(it);
					}
				}
				p00_inner += xstep;
	...
	std::vector<HaarResponse> badResult = haarResult;

	for (std::vector<HaarResponse>::iterator it = haarResult.begin(); it != haarResult.end(); ++it)
	{
		if (it->response > best_response * 0.4)   //remove it
		{
			it->raduis = -1;
			continue;
		}
		for (std::vector<HaarResponse>::iterator itb = badResult.begin(); itb != badResult.end(); ++itb)
		{
			if ((it->minValueIn.x - it->raduis) < (itb->minValueIn.x - itb->raduis)
				&& (it->minValueIn.y - it->raduis) < (itb->minValueIn.y - itb->raduis)
				&& it->minValueIn.x + it->raduis > itb->minValueIn.x + itb->raduis
				&& it->minValueIn.y + it->raduis > itb->minValueIn.y + itb->raduis)   // itb is included by it
			{
				it->raduis = -1;
			}
	...
	for (std::vector<HaarResponse>::iterator it = haarResult.begin(); it != haarResult.end(); ++it)
	{
		if (it->raduis > 0)
		{
			bflag = true;
			it->raduis *= 1.5;
			x1 = (it->minValueIn.x - it->raduis) < x1 ? it->minValueIn.x - it->raduis : x1;
			y1 = (it->minValueIn.y - it->raduis) < y1 ? it->minValueIn.y - it->raduis : y1;
			x2 = (it->minValueIn.x + it->raduis) > x2 ? it->minValueIn.x + it->raduis : x2;
			y2 = (it->minValueIn.y + it->raduis) > y2 ? it->minValueIn.y + it->raduis : y2;

			//test
	/*		cv::rectangle(mEye, cv::Rect(it->minValueIn.x - it->raduis, it->minValueIn.y - it->raduis, it->raduis * 2, it->raduis *2), cv::Scalar(255));
			cv::imshow("d", mEye);
			cv::waitKey(0);*/
			//cv::destroyAllWindows();
		}
	}
	if (false == bflag)  //by max 20170929   û�ҵ���roi�ᱻ��ֵĬ��ֵ1000
	{
		roi.x = 0;
		roi.y = 0;
		roi.width = 0;
...
std::shared_ptr<Detector2DResult> Detector2D::detect(Detector2DProperties& props, cv::Mat& image, cv::Mat& color_image, cv::Mat& debug_image, cv::Rect& roi, bool visualize, bool use_debug_image, bool pause_video)
{
	...
	if (roi.width < minROI || roi.height < minROI)
	{
		result->confidence = 0.0;
		return result;
	}
	...
	//std::cout << "pupil_image:" << pupil_image.cols << " " << pupil_image.rows << " " << lowest_spike_index + props.intensity_range << std::endl;
	...
	if (mUse_strong_prior){
		...
		if (!raw_edges.empty())
		{
			...
			if (support_ratio >= props.strong_perimeter_ratio_range_min){
				...
				if (visualize)
				{
					cv::Point center(50, 20);
					auto text_string = std::to_string(-result->confidence);
					cv::Size text_size = cv::getTextSize(text_string, cv::FONT_HERSHEY_SIMPLEX, 0.4, 1, 0);
					cv::Point text_pos = { center.x - text_size.width / 2, center.y + text_size.height / 2 };
					cv::putText(color_image, text_string, text_pos, cv::FONT_HERSHEY_SIMPLEX, 0.4, mRoyalBlue_color);
					cv::circle(color_image, cv::Point(result->ellipse.center[0], result->ellipse.center[1]), 5, cv::Scalar(255, 255, 255), 3);
				}
				return result;
			}
		}
	...
	//auto pruning_quick_combine = [&](const std::vector<std::vector<cv::Point>>& contours,  std::set<int>& seed_indices, int max_evals = 1e20, int max_depth = 5) {                         // by max 20170908
	auto pruning_quick_combine = [&](const std::vector<std::vector<cv::Point>>& contours, std::set<int>& seed_indices, int max_evals, int max_depth) {
